...
import { importClientsFromCsvForm } from "@/actions/clients";
import { Button } from "@/components/ui/button";

export function ClientImportForm() {
  const router = useRouter();
  const ref = useRef<HTMLFormElement>(null);
  const [state, action, pending] = useActionState(importClientsFromCsvForm, null);
  ...
  return (
    <div className="rounded-[var(--radius)] border border-border bg-card p-4">
      <h3 className="mb-2 text-sm font-semibold">ייבוא לקוחות מאקסל (CSV)</h3>
      <p className="mb-3 text-xs text-muted-foreground">אפשר להעלות קובץ CSV או XLSX לפי התבנית.</p>
